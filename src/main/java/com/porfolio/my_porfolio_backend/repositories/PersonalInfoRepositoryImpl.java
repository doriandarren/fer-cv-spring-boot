...
public class PersonalInfoRepositoryImpl implements IPersonalInfoRepository {
    ...
    private final RowMapper<PersonalInfo> personalInfoRowMapper = (rs, rowNum) -> {
        ...
        personalInfo.setProfileLinkedinUrl(rs.getString("profile_linkedin_url"));
    ...
    public PersonalInfo store(PersonalInfo personalInfo) {
        ...
        if(personalInfo.getId() == null){
            String sql = "INSERT INTO personal_info (first_name, last_name, title, profile_description, profile_image_url, years_of_experience, email, phone, profile_linkedin_url, github_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            jdbcTemplate.update(connection -> {
                ...
                ps.setString(9, personalInfo.getProfileLinkedinUrl());
            ...
            String sql = "UPDATE personal_info SET first_name = ?, last_name = ?, title = ?, profile_description = ?, profile_image_url = ?, years_of_experience = ?, email = ?, phone = ?, profile_linkedin_url = ?, github_url = ? WHERE id = ?";
            ...
            jdbcTemplate.update(sql, 
                ...
                personalInfo.getProfileLinkedinUrl(),
